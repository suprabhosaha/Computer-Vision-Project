...
class ColumnPooling:
    def __init__(self, num_columns: int = 5):
        self.Nc = num_columns  # Default number of columns
    ...
    def pool(self, image: np.ndarray, bboxes: List[Tuple[int, int, int, int]]) -> Dict[int, np.ndarray]:
        ...
        feature_size = component_features[0].shape[0] if component_features else 1
        for col in range(self.Nc):
            if col not in pooled_features:
                pooled_features[col] = np.zeros(feature_size)
